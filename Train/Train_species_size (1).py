...
def get_image_size_limit(path, species_lst=species_lst, folders=('train','valid'), exclude_folder=None, data_len=None):
    # dataset_path = r"/blue/hulcr/gmarais/Beetle_data/kfold_images/train_data"
    files_ext = get_image_files(path=path, recurse=True, folders=folders)
    results_df = pd.DataFrame()
    fold_results_dict = {}
    img_num_lst = []
    files_1 = []
    if exclude_folder:
        files_ext = L([f for f in files_ext if (exclude_folder not in str(f))])
    if data_len:
...
def get_images(dataset_path, batch_size, img_size, seed, subfolders=('train','valid'), exclude_folder=None, data_len=None):
    ...
    ################
    # files = get_image_size_limit(path=dataset_path, species_lst=species_lst, folders=subfolders, exclude_folder=exclude_folder, data_len=data_len)
    ################
    ...
    dblock = DataBlock(blocks = (ImageBlock, CategoryBlock),#(vocab=categories+["Unknown"])),
                       get_items = partial(get_image_files_exclude, 
                                          ...
                                          data_len=data_len),
                       ################
                       # get_items = partial(get_image_size_limit,
...
for i in range(1,folds+1):
    ...
    for data_len in range(1,data_length):
        ...
        files_2 = []
        for vial_sub in vial_subset_lst[:data_len]:
            files_vial_sub = [f for f in files if (vial_sub+"_" in str(f))]
            files_2 = files_2 + files_vial_sub
        img_num_lst.append(len(files_2))
        files =  L(files_1 + files_2)
...
#         files_1 = []
#         for species in species_lst:
#             files_ext = get_image_files(path=dataset_path, folders=('train_'+str(i)))
#             # filter files by size of data
#             files_temp = [f for f in files_ext if (species in str(f))]
#             vial_subset_lst = list(set([re.search(r'.*_([^_]+_.*[^_]+)_.*_.*', str(path)).group(1) for path in files_temp]))
#             random.seed(42)
